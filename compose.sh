...
RTX_5090_UUID=$(nvidia-smi --query-gpu=uuid,name --format=csv,noheader | \
    awk -F', ' '/RTX 5090/ {print $1}')
export RTX_5090_UUID

# 2. Extract the UUID for the RTX 4090
RTX_4090_UUID=$(nvidia-smi --query-gpu=uuid,name --format=csv,noheader | \
    awk -F', ' '/RTX 4090/ {print $1}')
export RTX_4090_UUID
